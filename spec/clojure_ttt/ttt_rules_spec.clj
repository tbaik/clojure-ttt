...
(describe "ttt-rules"
  ...
  (describe "game-over?"
    (it "returns true if has winner or board is full"
      (should= true
               (game-over? ["O" "X" "O" "X" "O" "X" "O" "X" 9]))
      (should= true
               (game-over? ["X" "O" "O" "O" "X" "X" "X" "O" "O"])))
    (it "returns false if does not have winner or board is not full"
      (should= false
               (game-over? ["O" "O" "X" "X" "O" "X" "O" "X" 9])))))
